...
def _extract_model_candidates(text: str) -> list[str]:
    candidates = re.findall(r"\b(?=[A-Z0-9-]{2,20}\b)(?=[A-Z0-9-]*\d)[A-Z0-9]+(?:-[A-Z0-9]+)*\b", text, flags=re.I)
    blocked = {"1600", "2024", "2025", "2026", "100", "120", "220", "240"}
    return _dedupe_strings([x for x in candidates if x.upper() not in blocked])
