...
15. The compatibility phrase alone is not enough localization. Independently rewrite the product noun, functions, scenarios, bullets and description for Amazon shoppers in {profile['language']}; do not translate the English wording line by line.
16. Before returning JSON, count the title characters. If it exceeds the limit, rewrite it as a shorter COMPLETE title. Never cut a title in the middle and never end it with a dangling preposition, conjunction or article.
17. English output must also be genuinely rewritten for search relevance; do not return the source title unchanged.
18. short_title must contain a useful combination of at least two source-supported highlights whenever the source provides them, rather than only repeating the material or product noun.
19. Do not introduce generic claims such as corrosion resistance, improved balance, stronger performance, professional grade or ideal use unless those exact facts are supported by SOURCE DATA.
""".strip()
