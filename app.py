...
def stable_cache_key(source: dict[str,Any], language: str) -> str:
    payload=json.dumps({"schema":"v2.1.1-ai-core-fix","language":language,"source":source},ensure_ascii=False,sort_keys=True)
...
def optimize_row_text(
    ...
    if cached:
        ...
        if ok:
            ...
            result.at[idx, "标题相似度"] = round(
                title_similarity(data["title"], source["title"]), 4
            )
            return True, ""

    # 1) Generate bullets + description once. Retry only this section when it fails.
    body: dict[str, str] = {}
    body_reason = ""
    ...
    for _ in range(max_attempts):
        try:
            ...
            body_ok, body_reason = validate_body(body, facts, cfg["compat"])
            if body_ok:
                break
            previous_body = body
        except Exception as exc:
    ...
    # 2) Generate title separately. Title failures never regenerate the body.
    title = ""
    title_reason = ""
    title_ok = False
    for _ in range(max_attempts):
        try:
            ...
            title_ok, title_reason = validate_title(
                title, facts, cfg["compat"], MAX_TITLE_LEN
            )
            if title_ok:
                break
        except Exception as exc:
    ...
    # 3) Generate Item Highlights separately. Highlight failures do not regenerate body/title.
    short_title = ""
    highlights_reason = ""
    highlights_ok = False
    for _ in range(max_attempts):
        try:
            ...
            highlights_ok, highlights_reason = validate_highlights(
                short_title, MAX_SHORT_TITLE_LEN
            )
            if highlights_ok:
                break
        except Exception as exc:
