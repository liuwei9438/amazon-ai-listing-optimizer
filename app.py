...
from core.product_analyzer import analyze_product, source_fingerprint
from core.body_generator import generate_body
from core.title_generator import generate_title
from core.highlights_generator import generate_highlights
...
def stable_cache_key(source: dict[str,Any], language: str) -> str:
    payload=json.dumps({"schema":"v2.1.2-pipeline-fix","language":language,"source":source},ensure_ascii=False,sort_keys=True)
...
def output_excel(df: pd.DataFrame) -> bytes:
    ...
    # Only fully optimized rows are exported. Failed rows remain visible in the
    # page failure list and can be retried, but are not mixed into the final file.
    if "优化状态" in export_df.columns:
        export_df = export_df[
            export_df["优化状态"].astype(str).eq("成功")
        ].copy()
...
def optimize_row_text(
    ...
    # Generated rows begin as copies of the source row. Clear all target copy
    # fields immediately, otherwise a failed AI task would retain and export the
    # original title/bullets, creating the false impression that AI did nothing.
    result.at[idx, title_col] = ""
    result.at[idx, short_title_col] = ""
    for column in bullet_cols:
        result.at[idx, column] = ""
    result.at[idx, desc_col] = ""

    result.at[idx, "语言"] = language
    result.at[idx, "__目标语言"] = language
    result.at[idx, "优化状态"] = "处理中"
    result.at[idx, "失败原因"] = ""
    result.at[idx, "产品分析状态"] = "等待"
    result.at[idx, "五点详情状态"] = "等待"
    result.at[idx, "标题状态"] = "等待"
    result.at[idx, "短标题状态"] = "等待"
    ...
    # Stage 1: product facts. One source product is analyzed once and reused by
    # every selected language.
    facts_key = source_fingerprint(source)
    facts = st.session_state.product_fact_cache.get(facts_key)
    if facts:
        result.at[idx, "产品分析状态"] = "缓存"
    else:
        try:
            facts = analyze_product(client, source)
            st.session_state.product_fact_cache[facts_key] = facts
            result.at[idx, "产品分析状态"] = "成功"
        except Exception as exc:
            reason = f"产品理解失败：{exc}"
            result.at[idx, "产品分析状态"] = "失败"
    ...
    if cached:
        data = local_repair(cached.copy(), facts, cfg["compat"])
        ok, reason = validate_listing(
            data,
            facts,
            cfg["compat"],
            MAX_TITLE_LEN,
        ...
        if ok:
            ...
            result.at[idx, short_title_col] = clean_text(data["short_title"])
            for i, column in enumerate(bullet_cols, start=1):
                result.at[idx, column] = clean_text(data[f"bullet{i}"])
            result.at[idx, desc_col] = clean_text(data["description"])
            result.at[idx, "产品分析状态"] = "缓存"
            result.at[idx, "五点详情状态"] = "缓存"
            result.at[idx, "标题状态"] = "缓存"
            result.at[idx, "短标题状态"] = "缓存"
            result.at[idx, "优化状态"] = "成功"
            result.at[idx, "失败原因"] = ""
            result.at[idx, "缓存命中"] = "是"
            result.at[idx, "SEO评分"] = seo_score(data, language)
            result.at[idx, "标题相似度"] = round(
                title_similarity(data["title"], source["title"]),
                4,
            )
            return True, ""

    # Stage 2: bullets and description. Successful stage output is written to the
    # dataframe immediately and is not regenerated when only title/highlights fail.
    body: dict[str, str] = {}
    body_reason = ""
    previous_body: dict[str, str] | None = None
    body_ok = False
    ...
    for _ in range(max_attempts):
        try:
            ...
            body_ok, body_reason = validate_body(
                body,
                facts,
                cfg["compat"],
            )
            ...
            body_reason = f"五点详情错误：{exc}"

    if not body_ok:
        reason = body_reason or "五点详情生成失败"
        result.at[idx, "五点详情状态"] = "失败"
        result.at[idx, "优化状态"] = "需重新优化"
        result.at[idx, "失败原因"] = reason
        return False, reason

    for i, column in enumerate(bullet_cols, start=1):
        result.at[idx, column] = clean_text(body[f"bullet{i}"])
    result.at[idx, desc_col] = clean_text(body["description"])
    result.at[idx, "五点详情状态"] = "成功"
    ...
    for _ in range(max_attempts):
        try:
            ...
            title_ok, title_reason = validate_title(
                title,
                facts,
                cfg["compat"],
                MAX_TITLE_LEN,
            ...
            title_reason = f"标题错误：{exc}"

    if not title_ok:
        reason = title_reason or "标题生成失败"
        result.at[idx, "标题状态"] = "失败"
        result.at[idx, "优化状态"] = "需重新优化"
        result.at[idx, "失败原因"] = reason
        return False, reason

    result.at[idx, title_col] = clean_text(title)
    result.at[idx, "标题状态"] = "成功"

    # Stage 4: Item Highlights only.
    short_title = ""
    highlights_reason = ""
    highlights_ok = False
    ...
    for _ in range(max_attempts):
        try:
            short_title = generate_highlights(
                client,
                facts,
                cfg["language"],
                MAX_SHORT_TITLE_LEN,
            ...
            highlights_ok, highlights_reason = validate_highlights(
                short_title,
                MAX_SHORT_TITLE_LEN,
            )
            if highlights_ok:
            ...
            highlights_reason = f"产品亮点错误：{exc}"

    if not highlights_ok:
        reason = highlights_reason or "产品亮点生成失败"
        result.at[idx, "短标题状态"] = "失败"
        result.at[idx, "优化状态"] = "需重新优化"
        result.at[idx, "失败原因"] = reason
        return False, reason

    result.at[idx, short_title_col] = clean_text(short_title)
    result.at[idx, "短标题状态"] = "成功"
    ...
    data = local_repair(
        {
            ...
            **{
                f"bullet{i}": clean_text(result.at[idx, column])
                for i, column in enumerate(bullet_cols, start=1)
            },
    ...
    success, reason = validate_listing(
        data,
        facts,
        cfg["compat"],
        MAX_TITLE_LEN,
    ...
    # Final normalized data is the only content marked successful.
    result.at[idx, title_col] = clean_text(data["title"])
    result.at[idx, short_title_col] = clean_text(data["short_title"])
    for i, column in enumerate(bullet_cols, start=1):
        result.at[idx, column] = clean_text(data[f"bullet{i}"])
    ...
    if st.button("开始多语言优化", type="primary", disabled=not selected_languages):
        ...
        for source_idx, source_row in source_df.iterrows():
            for language in selected_languages:
                ...
                row_dict[title_col] = ""
                row_dict[short_title_col] = ""
                for bullet_column in bullet_cols:
                    row_dict[bullet_column] = ""
                row_dict[desc_col] = ""
                row_dict["优化状态"] = "等待"
                row_dict["失败原因"] = ""
                row_dict["产品分析状态"] = "等待"
...
if st.session_state.result_df is not None:
    ...
    st.caption("导出的 Excel 仅包含完整优化成功的语言行；失败行不会夹带原标题进入导出文件。")
