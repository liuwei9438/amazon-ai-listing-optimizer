...
from core.listing_engine import generate_listing
from core.validator import local_repair, validate_listing

st.set_page_config(page_title="Amazon AI Listing Optimizer", layout="wide")

VERSION = "V2.1-Hybrid-AI-Core-Test"
MAX_TITLE_LEN = 75
...
def stable_cache_key(source: dict[str,Any], language: str) -> str:
    payload=json.dumps({"schema":"v2.1-hybrid-ai-core","language":language,"source":source},ensure_ascii=False,sort_keys=True)
...
def optimize_row_text(
    ...
    analysis_key = source_fingerprint(source)
    facts = st.session_state.product_fact_cache.get(analysis_key)
    if not facts:
        try:
            facts = analyze_product(client, source)
            st.session_state.product_fact_cache[analysis_key] = facts
    ...
    for attempt in range(max_attempts):
        try:
            raw = generate_listing(
                client=client,
                source=source,
                facts=facts,
                language_name=LANGUAGES[language]["language"],
                language_label=language,
                compatibility_phrase=LANGUAGES[language]["compat"],
                title_limit=MAX_TITLE_LEN,
                highlights_limit=MAX_SHORT_TITLE_LEN,
                retry_reason=reason,
    ...
    # Keep the last draft visible, but do not mechanically truncate it.
    if data:
        if data.get("title"):
            result.at[idx, title_col] = clean_text(data["title"])
    ...
    result.at[idx, "标题相似度"] = round(
        title_similarity((data or {}).get("title", ""), source["title"]),
        4,
    ) if data else ""
    result.at[idx, "缓存命中"] = "否"
