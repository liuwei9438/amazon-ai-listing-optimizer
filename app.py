...
from core.product_cache import product_fingerprint, result_fingerprint
from core.template_mapper import map_results_to_template, build_template_excel
from storage.result_cache import JsonResultCache
from storage.task_manager import TaskManager

st.set_page_config(page_title="Amazon AI Listing Optimizer", layout="wide")

VERSION = "V2.1-AI-Enhanced-20260717"
MAX_TITLE_LEN = 75
...
TEXT_DISK_CACHE = JsonResultCache("listing_results_v21_ai_enhanced_20260717")
ANALYSIS_DISK_CACHE = JsonResultCache("product_analysis_v21_ai_enhanced_20260717")
TASK_MANAGER = TaskManager("v21_ai_enhanced_20260717")
...
def stable_cache_key(source: dict[str,Any], language: str) -> str:
    return result_fingerprint(source, language, schema="v2.1-hybrid")
...
def optimize_row_text(
    ...
    source_fields = {
        str(column): clean_text(source_row.get(column, ""))
        for column in source_row.index
        if clean_text(source_row.get(column, ""))
    ...
    cached = get_session_dict("text_cache").get(cache_key) or TEXT_DISK_CACHE.get(cache_key)
    if cached:
        get_session_dict("text_cache")[cache_key] = cached
        data = cached.get("data", cached).copy()
        analysis_key = product_fingerprint(source)
        cached_analysis = (
            get_session_dict("product_analysis_cache").get(analysis_key)
            or ANALYSIS_DISK_CACHE.get(analysis_key)
            or cached.get("analysis", {})
        )
        profile = get_language_profile(language)
        data = deterministic_repair(data, profile, cached_analysis or {})
        success, reason, _ = validate_listing(data, source["title"], profile, cached_analysis or {})
        if success:
    ...
    analysis_key = product_fingerprint(source)
    analysis = get_session_dict("product_analysis_cache").get(analysis_key) or ANALYSIS_DISK_CACHE.get(analysis_key)
    if analysis:
        get_session_dict("product_analysis_cache")[analysis_key] = analysis
    try:
        existing = {
        ...
        if pipeline.get("analysis"):
            ...
            ANALYSIS_DISK_CACHE.set(analysis_key, pipeline["analysis"])
    ...
    if success and data:
        ...
        get_session_dict("text_cache")[cache_key] = cache_payload
        TEXT_DISK_CACHE.set(cache_key, cache_payload)
        return True, ""
...
def save_current_checkpoint(status: str = "running", completed: int | None = None, total: int | None = None) -> None:
    ...
    metadata = {
        "version": VERSION,
        "status": status,
        "completed": int(completed if completed is not None else len(result_df)),
        "total": int(total if total is not None else len(result_df)),
        "source_name": st.session_state.get("source_name", ""),
        "selected_languages": list(st.session_state.get("selected_languages", [])),
        "title_col": st.session_state.get("title_col"),
        "desc_col": st.session_state.get("desc_col"),
        "image_col": st.session_state.get("image_col"),
        "bullet_cols": list(st.session_state.get("bullet_cols", [])),
        "short_title_col": st.session_state.get("short_title_col", "短标题"),
        "fail_indices": list(st.session_state.get("fail_indices", [])),
        "image_fail_sources": list(st.session_state.get("image_fail_sources", [])),
        "logs": list(st.session_state.get("logs", []))[-2000:],
    }
    TASK_MANAGER.save(source_df=source_df, result_df=result_df, metadata=metadata)
...
def restore_checkpoint() -> bool:
    ...
    if not loaded:
        return False
    source_df, result_df, meta = loaded
    st.session_state.source_df = source_df
    st.session_state.result_df = result_df
    st.session_state.source_name = meta.get("source_name", "restored.xlsx")
    st.session_state.selected_languages = meta.get("selected_languages", ["英语"])
    st.session_state.title_col = meta.get("title_col")
    st.session_state.desc_col = meta.get("desc_col")
    st.session_state.image_col = meta.get("image_col")
    st.session_state.bullet_cols = meta.get("bullet_cols", [])
    st.session_state.short_title_col = meta.get("short_title_col", "短标题")
    st.session_state.fail_indices = refresh_fail_indices(result_df)
    st.session_state.image_fail_sources = meta.get("image_fail_sources", [])
    st.session_state.logs = meta.get("logs", [])
    st.session_state.restored_meta = meta
    return True


def revalidate_indices(indices: list[Any]) -> None:
    """Re-run local repair and QA without calling OpenAI."""
    if not indices:
        st.info("当前没有需要重新检测的记录。")
        return
    ...
    for pos, idx in enumerate(indices, start=1):
        ...
        current = {
            "title": clean_text(result.at[idx, title_col]),
            "short_title": clean_text(result.at[idx, short_title_col]),
            **{f"bullet{i}": clean_text(result.at[idx, c]) for i, c in enumerate(bullet_cols, start=1)},
            "description": clean_text(result.at[idx, desc_col]),
        }
        profile = get_language_profile(language)
        repaired = deterministic_repair(current, profile, analysis)
        ok, reason, score = validate_listing(repaired, source["title"], profile, analysis)

        result.at[idx, title_col] = repaired["title"]
        result.at[idx, short_title_col] = repaired["short_title"]
        for i, c in enumerate(bullet_cols, start=1):
            result.at[idx, c] = repaired[f"bullet{i}"]
        result.at[idx, desc_col] = repaired["description"]
        result.at[idx, "SEO评分"] = score
        if ok:
            result.at[idx, "优化状态"] = "成功"
            result.at[idx, "失败原因"] = ""
            passed += 1
    ...
    save_current_checkpoint(status="completed")
    st.success(f"重新检测完成：本次通过 {passed} 条，剩余失败 {len(st.session_state.fail_indices)} 条；未调用 AI。")
...
if st.session_state.result_df is None:
    saved = TASK_MANAGER.load()
    if saved:
        _, saved_result, saved_meta = saved
        saved_at = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(saved_meta.get("saved_at", 0)))
        st.info(f"检测到已保存任务：{len(saved_result)} 条结果，保存时间 {saved_at}")
        rc1, rc2 = st.columns(2)
        with rc1:
            if st.button("恢复上次优化结果", use_container_width=True):
                if restore_checkpoint():
                    st.rerun()
        with rc2:
            if st.button("删除已保存任务", use_container_width=True):
                TASK_MANAGER.clear()
                st.rerun()
    ...
    if st.button("开始多语言优化", type="primary", disabled=not selected_languages):
        ...
        for source_idx, source_row in source_df.iterrows():
            for language in selected_languages:
                ...
                idx = temp.index[-1]
                success, reason = optimize_row_text(client, temp, idx, source_row, title_col, bullet_cols, desc_col, short_title_col, language, max_attempts=3, retry_round=0)
                generated_rows[-1] = temp.loc[idx].to_dict()
                if done_tasks % CHECKPOINT_EVERY == 0 or done_tasks == total_tasks:
                    partial_result = pd.DataFrame(generated_rows).fillna("").reset_index(drop=True)
                    st.session_state.source_df = source_df
                    st.session_state.result_df = partial_result
                    st.session_state.fail_indices = refresh_fail_indices(partial_result)
                    ...
                    save_current_checkpoint(status="running", completed=done_tasks, total=total_tasks)
                sku = clean_text(source_row.get(sku_col, "")) if sku_col else str(source_idx)
        ...
        save_current_checkpoint(status="completed", completed=total_tasks, total=total_tasks)
...
if st.session_state.result_df is not None:
    ...
    if st.session_state.fail_indices:
        ...
        with b1:
            if st.button("重新检测全部失败项（不耗 Token）", use_container_width=True):
                revalidate_indices(list(st.session_state.fail_indices))
                st.rerun()
        with b2:
    ...
    st.caption("结果每完成5条自动保存。浏览器刷新后可从页面顶部恢复。说明：Streamlit Cloud 在重新部署或容器重建时可能清除本地检查点。")
    if st.button("立即保存当前任务", use_container_width=True):
        save_current_checkpoint(status="completed")
        st.success("当前结果已保存。")

    template_path = Path(__file__).resolve().parent / "templates" / "714导入产品模板.xlsx"
    template_df = map_results_to_template(
        result_df,
    ...
    st.caption(
        "导出严格使用固定模板列和顺序。源表没有的重量、包装尺寸、材料等字段保持空白；"
        "源表 sku 写入父SKU，child 写入SKU。仅导出优化成功的语言行。"
    )
    st.download_button(
        "按714固定模板导出",
        data=build_template_excel(template_df, template_path),
